...
class UndercoverGame {
    ...
    init() {
        ...
        this.loadWordPairs().then(() => {
            ...
            alert('Erreur lors du chargement des mots. Veuillez vérifier le fichier words.json.');
        });
    }
    ...
    selectPlayerCount(count) {
        ...
        const selectedButton = document.querySelector(`.count-btn[data-count="${count}"]`);
        if (selectedButton) {
    ...
    startGame() {
        ...
        this.players.forEach((player, index) => {
            player.word = index === undercoverIndex ? this.selectedWordPair[1] : this.selectedWordPair[0];
